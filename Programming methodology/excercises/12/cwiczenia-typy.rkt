...
(define (prefixes xs)
  ...
  (define (help xs acc)
    (if (null? xs)
        xs
        (cons (reverse acc) (help (cdr xs) (cons (car xs) acc)))))
  (append (cdr (help xs null)) (list xs)))

;cw7

(define-type Leaf 'leaf)
(define-type (Node V L) (List 'node V L))
(define-type (Tree T) (U Leaf (Node T T)))

(define-predicate leaf? Leaf)
(define-predicate node? (Node Integer Any))
...
(: node-subtrees (All (A B) (-> (Node A B) B)))
...
;(: print-tree (All (A) (-> (Tree A) (U Null A (Listof Integer)))))
;(define (print-tree tree)
;  (cond
;    [(leaf? tree) null]
;    [(leaf? (node-subtrees tree)) (node-val tree)]
;    [else (flatten (append (list (node-val tree)) (map (lambda (x) (print-tree x)) (node-subtrees tree))))]))
