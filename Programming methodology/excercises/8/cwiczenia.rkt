...
(define (car/cdr? x)
  (define (check-car/cdr xs)
    (cond
      [(if (null? (cdr xs))
           (eq? (car xs) 'r)
           #f)]
      [(or (eq? (car xs) 'a) (eq? (car xs) 'd)) (check-car/cdr (cdr xs))]
      [else #f]))
  (if (and (list? x) (= (length x) 2))
      (let ((xs (split-symbol (car x))))
        (and (eq? 'c (first xs))
             (>= (length xs) 3)
             (check-car/cdr (cdr xs))))
      #f))

(define (build-car/cdr x)
  (define (help symbols x)
    (cond
      [(eq? 'r (car symbols)) (cadr x)]
      [(eq? 'a (car symbols)) (car (help (cdr symbols) x))]
      [(eq? 'd (car symbols)) (cdr (help (cdr symbols) x))]))
  (help (cdr (split-symbol (car x))) x))
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(listt? e)                
         (map (lambda (a) (eval-env a env)) (listt-elements e))]
        [(car? e)
         (car (eval-env (car-expr e) env))]
        [(cdr? e)
         (cdr (eval-env (cdr-expr e) env))]
        [(car/cdr? e)
         (build-car/cdr (list (car e) (eval-env (cadr e) env)))]
        [(pair?? e)
...
(define (fibs-from m n)
  (lcons m (lambda () (fibs-from n (+ m n)))))

(define fibs (fibs-from 0 1))

(define (ints-from l count)
  (if (odd? count)
      (lcons
       l
       (lambda () (ints-from (- l 1) (+ count 1))))
      (lcons
       (* l -1)
       (lambda () (ints-from l (+ count 1))))))
...
(define (listt-elements t)
  (cdr t))

(define (my-nll? x)
  (eq? x null))

;append (dodalem null? i listy)
(eval '(let (x (lambda-rec (append xs ys)
                           (if (null? xs)
                               ys
...
;map
(eval '(let (x (lambda-rec (map f xs)
                           (if (null? xs)
                               xs
                               (cons (f (car xs)) (map f (cdr xs))))))
         (x (lambda (x) (+ x 1)) (list 1 2 3 4))))

;reverse

(eval '(let (x (lambda-rec (reverse xs)
                           (if (null? xs)
                               xs
                               (cons (reverse (cdr xs)) (car xs)))))
         (x (list 1 2 3))))
;dobry reverse
(eval '(let (y (lambda-rec (iter xs acc)
                          (if (null? xs)
                              acc
                              (iter (cdr xs) (cons (car xs) acc)))))
        (let (x (lambda-rec (reverse xs)
                            (y xs null)))
         (x (list 1 2 3)))))
...
(eval '(caddddr (list 1 2 3 4 5 6 7 8 9)))
(eval '(cdddddr (list 1 2 3 4 5 6 7 8 9)))
