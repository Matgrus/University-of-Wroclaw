...
(define (solve-ax+by=1 a b)
  (define (egcd x y)
    (if (= x 0)
        (list y 0 1)
        (let ([wyn (egcd (modulo y x) x)])
          (let ([g (car wyn)]
                [h (car (cdr wyn))]
                [i (car (cdr (cdr wyn)))])
            (list g (- i (* (floor (/ y x)) h)) h)))))
  (let ([wyn (egcd a b)])
        (let ([g (car wyn)]
              [h (car (cdr wyn))]
              [i (car (cdr (cdr wyn)))])
          (if (= g 1)
              (cons 1(modulo h b))
              (error "nie sa wzglednie pierwsze")))))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ([x (RSA-encrypt message recipient-public-key)])
    (let ([y (RSA-transform (compress x) sender-private-key)])
    (make-signed-message x y))))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (if (= (RSA-transform (signature cyphertext) sender-public-key) (compress (message cyphertext)))
  (RSA-decrypt (message cyphertext) recipient-private-key)
  (error "niezgodny podpis cyfrowy!")))
...
#|
(RSA-encrypt "ala ma kota" (key-pair-public test-key-pair1))
(RSA-decrypt (RSA-encrypt "ala ma kota" (key-pair-public test-key-pair1)) (key-pair-private test-key-pair1))

(solve-ax+by=1 3 7)
(solve-ax+by=1 34561 126534614627)

(encrypt-and-sign "metody programowania" (key-pair-public test-key-pair1) (key-pair-private test-key-pair2))
(authenticate-and-decrypt (encrypt-and-sign "metody programowania" (key-pair-public test-key-pair1) (key-pair-private test-key-pair2))
