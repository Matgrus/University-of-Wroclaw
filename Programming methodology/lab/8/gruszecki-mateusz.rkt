...
(define (expr? t)
  (or (const? t)
      ...
      (and (cons? t)
           ...
           (expr? (cons-snd t)))
      (and (arith-list? t)           
           (expr? (first (arith-list-elements t)))
           (expr? (second (arith-list-elements t)))
           (expr? (third (arith-list-elements t))))
      (and (car? t)
           (expr? (car-expr t)))
      (and (cdr? t)
           (expr? (cdr-expr t)))
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(arith-list? e)                
         (list (eval-env (first (arith-list-elements e)) env)
               (eval-env (second (arith-list-elements e)) env)
               (eval-env (third (arith-list-elements e)) env))]
        [(car? e)
...
(define (arith-list? t)   ;rozbudowalem nasz jezyk o trzyelementowe listy ulatwiajace testowanie
  (tagged-list? 'list t))

(define (arith-list-elements t)
  (cdr t))


;ewaluator wyrazen arytmetycznych

(eval '(let (x (lambda-rec (arith-eval expr)
                     (cond
                       [(pair? expr) (cond
                                       [(eq? (car expr) (quote +)) (+ (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
                                       [(eq? (car expr) (quote -)) (- (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
                                       [(eq? (car expr) (quote /)) (/ (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
                                       [(eq? (car expr) (quote *)) (* (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
                                       )]
                       [true expr])))
         (x (list (quote +) (list (quote /) 87 3) 4.5))
         ))

;testy

(define (my-arith-eval ls)
    (quasiquote (let (x (lambda-rec (arith-eval expr)
                     (cond
                       [(pair? expr) (cond
                                       [(eq? (car expr) (quote *)) (* (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
                                       [(eq? (car expr) (quote /)) (/ (arith-eval (car (cdr expr))) (arith-eval (car (cdr (cdr expr)))))]
...
(define test1 '(list (quote *) 4.5 12))
(define test2 '(list (quote *) (list (quote /) 2 (list (quote +) 5 8)) (list (quote -) 0 67)))
(define test3 '(list (quote /) 1024 (list (quote /) 512 (list (quote /) 256 (list (quote /) 128 (list (quote /) 4 2))))))
(eval (my-arith-eval test1))
(eval (my-arith-eval test2))
(eval (my-arith-eval test3))
