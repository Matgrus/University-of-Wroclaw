...
(define fermat-test
  '( (composite := false)
     (while (> k 0)
            ( (a := (+ 2 (rand (- n 4))))
              (if (= (mod (pow a (- n 1)) n) (mod 1 n))  ; rozbudowałem język while o operację pow (potęgowanie)
                  (k := (- k 1))
                  ( (composite := true)
                    (k := 0) )))))
  )
...
(probably-prime? 5 2)
(probably-prime? 71 50)
(probably-prime? 3691 10000)
(probably-prime? 2362578 10000)
(probably-prime? 31957 10000)
