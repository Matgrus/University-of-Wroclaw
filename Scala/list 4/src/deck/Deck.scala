...
class Deck(cards: List[Card]) {
  ...
  def push(color: String, value: String):Deck = {
    require(List("2", "3", "4", "5", "6", "7", "8", "9", "10", "King", "Queen", "Ace", "Jack").contains(value) &&
      List("Diamonds", "Spades", "Hearts", "Clubs").contains(color), "card error")
  ...
  def amountOfColor(color: String): Int = {
    require(List("Diamonds", "Spades", "Hearts", "Clubs").contains(color), "wrong color")
    cards.count(_.get_color() == color)
  }

  def amountOfNumerical(numerical: String): Int = {
    require(List("2", "3", "4", "5", "6", "7", "8", "9", "10").contains(numerical), "wrong numerical")
    cards.count(_.get_value() == numerical)
  }

  val amountWithNumerical: Int = {
    cards.count(x => List("2", "3", "4", "5", "6", "7", "8", "9", "10").contains(x.get_value()))
  }
...
object Deck{
  ...
  def apply(): Deck = {
    ...
    val vals = List("2", "3", "4", "5", "6", "7", "8", "9", "10" , "Jack", "Queen", "King", "Ace")
