...
def isPrime(n: Int): Boolean = {
  if(n<2){
    ...
    (2 to math.sqrt(n).toInt+1) forall (i => n % i != 0)
  }
}
...
def fileLines(file: java.io.File): List[String] = {
  val lines = (for (line <- io.Source.fromFile(file).getLines()) yield line).toList
  //stream.close()
  lines
}
...
def printNonEmptyUgly(pattern: String): Unit = {
  var i = 0
  do{
    if(filesHere(i).getName.matches(pattern) && fileLines(filesHere(i)).nonEmpty){
